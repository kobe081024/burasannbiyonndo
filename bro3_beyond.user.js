	...
	if (location.pathname === "/facility/castle_send_troop.php") {
		...
		if (g_beyond_options[MAP_11]) {
			...
			for (var i = 0; i < generalEnts.length; i++) {
				...
				all = generalEnts.eq(i).html().match(/(獲|大徳|劫略|攻奪|収奪|趁火打劫|桃賊の襲撃)/);
				var hasPrize = false;
				if (all != null) {
					hasPrize = true;
				}
				...
				if (is_active == true) {
					el.before(
						"<div>" +
							...
							"<input type='button' id='deck_set_" + i + "' style='font-size: 13px;' value='配置'></input>" +
					...
					el.before(
						"<div>" +
							...
							"<input type='button' style='font-size: 13px;' value='配置' disabled></input>" +
